...
public class EPKeyMapCM extends EPMapCM {
	...
	public static final int EXIT = CURSEUR.nextVal();
	public static final int ESCAPE = CURSEUR.nextVal();
	...
	public static final int SHIFT = CURSEUR.nextVal();
	...
	protected void implLoad() throws KeyContainerAlreadyUsed {
		register(new EPMapCE(ESCAPE, "ESCAPE", EPConfigCM.KEYBOARD_ESCAPE));
		...
		register(new EPMapCE(SHIFT, "SHIFT", EPConfigCM.KEYBOARD_SHIFT));
	...
	public Orientation getByKeyMap() {
		...
		if (actives.size() == 2) {
			result = Orientation.getConbinaison(
