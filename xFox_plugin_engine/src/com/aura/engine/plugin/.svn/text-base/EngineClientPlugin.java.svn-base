		...
		if (ap.getId() == EPPacketCM.RESULT_TOKEN) {
			...
			if (univers == null) {
				univers = new UniversClient(getAura()) {
					@Override
					public WorldClient initDefaultWorld() {
						return new WorldClient(getAura(), this, -1l);
					}
				};
			}
			univers.setSelf(new EntityTokenClient(p.getTokenId()));
	...
	private void whileTokenLoaded(final AbstractPacketManager pm, final AbstractPacket ap) {
		if (ap.getId() == EPPacketCM.RESULT_WORLD) {
			...
			if (univers.getFocusWorld() == null) {
				univers.setFocusWorld(univers.getDefaultWorld());
				screen = createScreen(getAura());
				screen.getEngine().play();				
			}
			univers.setFocusWorld(new WorldClient(getAura(), univers, p.getWorldId()));
...
//			}.start();
	...
	public void requestSpawn(int count) {
		...
		AuraLogger.info(getAura().getSide(), "Demande de spawn, count: " + count);
