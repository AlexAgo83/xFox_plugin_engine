	...
	private void onMove(AbstractPacket ap, EntityTokenServer token) {
		...
		for (EntityElementMove me: mes) {
			...
			if (drawable == null) {
				...
				drawable = new DEntity<AuraServer>(getAura(), id, dg, me.getName(), me.getLocation(), null);
				token.getWorld().attach(drawable);
				AuraLogger.info(getAura().getSide(), 
...
//			me.setSpeed(drawableEntity.getSpeed()); // init par le joueur
			...
			switch (me.getMoveElementType()) {
				...
				case TARGET : 
					...
					drawableEntity.move(met.getTarget(), me.getSpeed()); 
					break;
			}
		}
	...
	public void onCommand(String[] cmd) {
		...
		if (cp.getId() == EPConsoleCM.ADD_WORLD) {
			try {
				...
				WorldServer w = getUnivers().createNewWorld(newSeed);
