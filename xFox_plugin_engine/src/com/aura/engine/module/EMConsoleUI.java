...
//							System.out.println(getAura().getSide()+ "Err.", e);
							e.printStackTrace();
						}
					}
				}
				...
				console.add((info != null ? 
						(side == TypeSide.SERVER ?  
							...
							AuraBaliseParser.COLOR.getBalise(colorSystemHighlite) + "] "
							: "") 
//						+ AuraBaliseParser.COLOR.getBalise(colorSystemText) + info + 
//						AuraBaliseParser.COLOR.getBalise(colorSystemHighlite) + " " 
					: "") + 
	...
	public void doDraw(Graphics2D g) {
		...
		g.drawString(champSaisie, 5, getScreen().getView().getHeight()-5);
	}
	...
	public void doUpdate() {
		...
		if (currentFade >= .1f && markAsDraw) {
			...
			for (int i=cs.length-1; i >= 0; i--) {
				...
				if (frame != null) {
					consoleToDraw.put(
						getScreen().getView().getHeight() - 20 - (cpt*11), 
